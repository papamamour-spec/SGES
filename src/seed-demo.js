/* JEU DE DÉMONSTRATION (optionnel) : périmètre Groupe EDK simulé, comptes par rôle,
   référentiels et données d'exemple pour dérouler le cas d'usage imposé du CDC (§16.2).
   La production démarre vide : voir src/seed.js (compte administrateur seul). */
...
/* Jeu de DÉMONSTRATION uniquement : à lancer à la main via `npm run seed:demo`
   sur un environnement de démo (jamais appelé automatiquement en production).
   Refuse de s'exécuter si la base contient déjà des entités. */
function seedDemo() {
...
if (require.main === module) {
  console.log(seedDemo()
    ? 'Jeu de démonstration chargé : périmètre EDK, comptes de démonstration (mot de passe : edk2026) et données d’exemple.'
    : 'La base contient déjà des entités : jeu de démonstration NON chargé.');
}
